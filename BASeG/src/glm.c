...
int muvalid(int family, double mu) {
  double minb = 0.0, maxb =1, minp = 0.0 ;
  double gammaMin = 0.0 ;
  switch (family) {
           ...
           double *scale, int *df_resid, double* beta) 
{
  ...
  for (i=0; i<N; i++) {
    ...
    if (!muvalid(family, mu)) {
      wi = ri = 0.0;
			Rprintf("Nu1 muvalid i=%d y=%f mu=%f\n",i,y[i], mu);
    }else {
		  Nu ++;
          ...
          double *scoreTestP, int *trace, double *beta)
{
  ...
  *twologlik = 2.0*Lm;
  ...
  double *nX_j, *X_jk, *Xnew, **Xnew2;

	Xnew   = (double *)Calloc(N*(M+1), double);
  qty   = (double *)Calloc(N, double);
  qraux = (double *)Calloc(M+1, double);
  coef  = (double *)Calloc(M+1, double);
  work  = (double *)Calloc(2*(M+1), double);
  jpvt  = (int *)Calloc(M+1, int);
	...
	for(i=0; i<N; i++) {
    Xnew[i] = sqrt(weights[i]);
    for(k=0; k<M; k++) 
      Xnew[i+(k+1)*N] = sqrt(weights[i])*X[i+(k)*N];
  }
		/*reorg(Xnew, &Xnew2, N, M+1);
    ...
    Rprintf("wX\n");
    Rprint_me(Xnew2, 0, 2, 0, 9);*/


    F77_CALL(dqrls)(Xnew, &N, &pp, wz, &ny, &tol, coef, resid,
                    qty, &rank2, jpvt, qraux, work);

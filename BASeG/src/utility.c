...
/**********************************************************************
 * 
 * sample covariance between y1 and y2 (added by naim)
 *
 * calculate cov of y1[nl:nh] and y2[nl:nh], given mean_x
 *
 * do not use MLE of sd, i.e. 
 * cov(x) = sum((y1 - mean_y1)*(y2 - mean_y2))/(n-1)
 *
 **********************************************************************/

double cov(double* x, double* y, int nl, int nh, double mean_x,double mean_y)
{
  double sum2=0.0, t=0.0, t2=0.0;
  int i;
  int n = nh - nl + 1;
  for(i=nl; i<=nh; i++){
    t = x[i] - mean_x;
...
/**********************************************************************
 * 
 * chol,  returns column stacked choleskey decomposition for 2x2 matrix. 
 * Input matrix is also 2x2 column stacked vector. 
 *
 **********************************************************************/

void chol(double *mat, double *result){
	double a, b, c;	
	a = sqrt(mat[0]); //mat[1] > 0
	b = mat[1]/a;
	//if diff less than 0, set to 0?
	if(mat[3]> b*b){
		c = sqrt(mat[3] - b*b);
	}else{
		c=pow(10,-100);
...
/**********************************************************************
 * 
 * det,  returns determinant for 2x2 matrix. 
 * Input matrix is also 2x2 column stacked vector. 
 *
 **********************************************************************/

double det(double *mat){
	double val = mat[0]*mat[3] - mat[1]*mat[2];
	if(val == 0) val = 10^-100;
	return(val);
}
...
/**********************************************************************
 * 
 * inv,  returns inverse for 2x2 matrix. 
 * Input matrix is also 2x2 column stacked vector. 
 *
 **********************************************************************/

void inv(double *mat, double *result){
	double d = det(mat);
	result[0] = mat[3]/d;
	result[1] = -mat[1]/d;
	result[2] = -mat[2]/d;
